...
function balanceOf(account_from, callback) { 
...
function transfer(account_from, account_to, amount, callback) {
	/* constants */
	...
	web3.eth.personal.unlockAccount(account_from, account_password, 60, function (error, result) {
		...
		var contract_data = contract.methods.transfer(account_to, amount).send({from: account_from}).then().catch(console.error);
...
app.get('/getbalance/:account', function(request, response, next) {
	var account = request.params.account;
	// 0x7c065c3068d27ae3d1b04129333414d37c5ad076
	balanceOf(account, function(balance){
		response.send(JSON.stringify({ balance: balance }));
	});
  
});

app.get('/sendfrom/:account_from/:account_to/:amount', function(request, response, next) {
	var account_from = request.params.account_from;
	var account_to = request.params.account_to;
	var amount = request.params.amount;
	transfer(account_from, account_to, amount, function(result){
